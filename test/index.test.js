...
const ajvSchema = {
    properties: {
        "_id": { type: "string", pattern: '^[0-9a-fA-F]{24}$', example: "54d9f4fd1671547cb016a205" },
        ...
        "birthday": { type: "string", format: 'date-time', example: '2023-03-06T10:30:00.000+0000Z' },
        "ts": { type: "timestamp" },
...
test('Required', async t => {
    t.same(mooSchema.name, { type: 'String', required: true }, "required if required")
    t.notHas(mooSchema.age, 'required', "required not present if not required" );
    t.same(mooSchema.car.id, { type: 'Number', required: true }, "required if required in subdocument");
    t.notHas(mooSchema.car.model, 'required', "required not present if not required in subdocument");
});